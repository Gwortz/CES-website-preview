...
export default function YouTubeEmbed({
  ...
  if (playing) {
    return (
      <div
        className={`relative aspect-video overflow-hidden rounded-2xl border border-brand-900/10 bg-brand-950 ${className}`}
      >
        <iframe
  ...
  return (
    <button
      ...
      className={`group relative block aspect-video w-full overflow-hidden rounded-2xl border border-brand-900/10 bg-brand-950 text-left ${className}`}
    >
      ...
      <span
        aria-hidden="true"
        className="absolute left-1/2 top-1/2 flex h-16 w-16 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-accent-600 shadow-lg transition-transform group-hover:scale-110"
      >
